...
%        0.0311    0.8449=-+2    0.2358; 0.1307    0.8383    0.2366; 0.3935    0.8144    0.2492;
...
circle_radius = 0.1;
waypoints = test16(circle_radius,circle_centre,height);
% waypoints(:,3) = -0.8;
origin= [0,0,0];
origin1 = [-0.1,0.35,0];
x_point = [1,0,0];
...
% Robot frame
frame = create_frame1(origin,x_point,y_point);

% Tilted object frame
frame1 = create_frame1(origin1,x_point1,y_point1);

% waypoints_trans = circle_transformtest(frame,waypoints);

waypoints_trans = circle_transform(frame1,frame,waypoints);
% t = transfrom_list_to_world_frame(frame1,waypoints)
% waypoints_trans = circle_transform(frame1,robotframe,waypoints)
% con=out.con;
...
% Set robot positiooutn to first waypoint for orientation setting
